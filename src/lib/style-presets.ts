...
export const V2_STORYBOARD_BRIEF =
  ...
  "\n" +
  "CHAQUE PLAN EST UNE MÉTAPHORE VISUELLE de sa phrase, construite avec un ou deux objets en papier clairs — jamais une illustration littérale d'une personne en train de faire l'action. Une idée abstraite devient un objet (le temps → une horloge ajourée ; la mémoire → une pile de livres ; un danger → une flèche qui tombe).\n" +
  "\n" +
  ...
  "\n" +
  "MOUVEMENT DÉCIDÉ PAR LE SENS : une phrase qui révèle, donne un chiffre, un danger ou un choc → RAPIDE (chute, éclatement, zoom brusque, fouetté de caméra, couches qui claquent) ; une phrase qui explique un mécanisme → MOYEN et continu (une chose se déplace, s'ouvre, se transforme) ; une phrase de conclusion ou d'émotion → LENT et ample. Chaque videoPrompt commence par « FAST: », « STEADY: » ou « SLOW: ». QUOTA : au moins cinq FAST par vidéo, jamais plus de trois SLOW. Dans chaque videoPrompt, un objet ou une couche entière de papier TRAVERSE au moins un tiers du carré et la caméra bouge aussi (push-in, tilt, pan, pull-back) — une lumière qui change ou un parallaxe seul ne compte pas. Quand la phrase enchaîne sur la suivante, termine le videoPrompt par une SORTIE (le sujet quitte le cadre, une couche recouvre la scène, la caméra dépasse le sujet) pour que la coupe fasse transition. Le videoPrompt n'anime que ce qui est dans l'imagePrompt, en 8 secondes maximum.\n" +
  "\n" +
  "VARIÉTÉ OBLIGATOIRE : deux plans qui se suivent ne partagent jamais le même cadrage ni la même échelle. Fais tourner, dans l'ordre qui colle au texte : PAYSAGE LARGE avec horizon et silhouette minuscule · OBJET SEUL énorme sur fond de papier uni · TRÈS GROS PLAN sur un détail qui remplit le carré · VUE DU DESSUS à plat · SILHOUETTE À CONTRE-JOUR devant un fond lumineux · COUPE ou SCHÉMA en couches de papier · SCÈNE DE NUIT (fond sombre) alternée avec SCÈNE DE JOUR (fond clair). Jamais de noms propres d'œuvres, de marques ou de personnages protégés dans les prompts : décris ce qu'on voit.\n" +
  "\n" +
