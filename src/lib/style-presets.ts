...
export const DEFAULT_MOTION: Record<VisualStyleId, string> = {
  ...
  papercraft_v2:
    "Smooth continuous paper animation with ONE clearly visible action in every shot: the red silhouette walks, turns or reaches; an object falls, opens, spins, drifts or slides into place; paper layers part or close; a steady camera push, pan or tilt across the set at the same time. Something must be moving during the whole clip, from the first frame to the last — never a static tableau with only parallax. The motion stays fluid and steady, rendered at full frame rate with natural motion blur: no stop-motion stepping, no jitter, no shaking, no strobing.",
