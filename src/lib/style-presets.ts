...
export const DEFAULT_VISUAL_BRIEF: Record<VisualStyleId, string> = {
  ...
  papercraft_v2:
    "handmade layered paper cut-out diorama, 4 to 6 stacked planes of matte construction paper with torn deckled edges, visible paper grain and crisp cast shadows between the layers, ONE warm directional key light from the upper left. THE SCENE FILLS THE WHOLE SQUARE EDGE TO EDGE: no paper frame, no border, no box, no vignette, no shadow-box around the scene — the paper landscape runs off all four edges of the image. STRICT PALETTE for the whole video: two dominant paper tones chosen once for the whole story (for example deep navy + slate, or cream + sand), one light cream paper, and a single vivid red — nothing else, no gradients, no 3D render look. HUMAN FIGURES ARE ALWAYS FLAT FACELESS PAPER SILHOUETTES: no eyes, no mouth, no facial features, one simple bold body shape; when a person appears it IS the single red element of the shot. Every shot is a VISUAL METAPHOR of its sentence built from one or two clear paper objects — never a literal illustration of a person doing the action. No text, no letters, no numbers, no logos.",
  cinematique:
...
export const DEFAULT_MOTION: Record<VisualStyleId, string> = {
  ...
  papercraft_v2:
    "Paper animation whose INTENSITY FOLLOWS THE SHOT'S videoPrompt: when it calls for a fast action (a drop, a burst, a snap zoom, a whip pan, layers slamming shut), make it fast and punchy; when it calls for a steady action (walking, sliding, opening, a slow push-in), keep it smooth and continuous. In every case something is clearly moving from the first frame to the last — never a static tableau with only parallax. When the videoPrompt ends with an exit (the subject leaves the frame, a layer covers the scene, the camera pushes past the subject), finish the clip on that exit so the cut to the next shot feels like a transition. Motion is rendered at full frame rate with natural motion blur: no stop-motion stepping, no jitter, no strobing, no shaking of the whole image. The one thing that moves is the action the sentence describes; nothing else moves for decoration, and the camera only follows it gently.",
...
export const DEFAULT_HOOK_BRIEF = [
  ...
  "",
  "LA MÉCANIQUE, VALIDÉE PAR LE CLIENT — ELLE PRIME SUR TOUT LE RESTE :",
  "• LE HOOK PREND LA RÉPONSE QUE LE SPECTATEUR A DANS LA TÊTE ET LA LUI RETIRE. C'est tout le mécanisme. Le spectateur croit savoir ; la première phrase lui enlève sa réponse, la seconde lui montre où chercher la vraie.",
  "• STRUCTURE OBLIGATOIRE : phrase 1, on RETIRE LA CROYANCE. Phrase 2, on ANNONCE LA VRAIE PISTE. Toujours DEUX phrases courtes, jamais une seule, jamais trois.",
  "• Environ 20 à 30 MOTS au total, soit les 6 à 8 premières secondes.",
  "• TOUJOURS COMPRÉHENSIBLE SANS AUCUNE CONNAISSANCE PRÉALABLE. Si le spectateur doit déjà savoir quelque chose pour comprendre le hook, il est raté.",
  "",
  "LES SIX HOOKS DE RÉFÉRENCE VALIDÉS (le niveau exact à atteindre) :",
  "• « En été, la Terre est plus LOIN du Soleil qu'en hiver. Les saisons ne viennent pas de là. »",
  "• « Les feuilles ne changent pas de couleur en automne. Le jaune était là depuis le printemps. »",
  "• « Napoléon n'a pas perdu Waterloo à cause des Anglais. Il a perdu à cause de la pluie. »",
  "• « Le talon d'Achille n'existe pas chez Homère. Il n'en parle pas une seule fois. »",
  "• « Tu ne dors pas pour te reposer. Tu dors pour que ton cerveau se nettoie de ses déchets. »",
  "• « Quand tu as des frissons, ton corps essaie de dresser une fourrure que tu n'as plus. »",
  "",
  "• TON AFFIRMATIF ET DIRECT. Aucune question rhétorique, aucun « saviez-vous », aucune mise en contexte avant l'affirmation. Jamais « Imagine », « Voici », « Selon », jamais une date en ouverture, jamais un nom propre qu'il faudrait expliquer.",
  "• TUTOIEMENT dès que le sujet touche personnellement le spectateur (« Tu ne dors pas pour te reposer. »).",
  "• Le hook PEUT donner la conclusion : la tension ne porte pas sur QUOI mais sur COMMENT. On reste pour le mécanisme.",
  "",
  "TU PROPOSES TROIS accroches candidates dans hookOptions, toutes construites sur cette mécanique. Tu NOTES chacune dans hookScores (une ligne par candidate : deux phrases ? la phrase 1 retire-t-elle vraiment une croyance que le spectateur a ? la phrase 2 annonce-t-elle la vraie piste ? 20-30 mots ? compréhensible sans connaissance préalable ? ton affirmatif ?). Tu gardes la meilleure : elle devient le champ hook ET la narration du plan 1, mot pour mot. Tu expliques ton choix en UNE ligne dans hookChoice.",
...
export const DEFAULT_STRUCTURE_BRIEF = [
  "STRUCTURE VALIDÉE PAR LE CLIENT — CHAQUE PLAN A UNE FONCTION, AUCUN N'EST DÉCORATIF (adapte les proportions si le nombre de plans diffère de huit, mais garde l'ordre) :",
  "• Plan 1 : LE HOOK QUI RETIRE LA CROYANCE — deux phrases courtes (voir les règles d'accroche).",
  "• Plans 2-3 : LE CONSTAT CONCRET QUI PROUVE QUE LA CROYANCE EST FAUSSE, avec un chiffre comparé à quelque chose de familier.",
  "• Plans 4-5 : LE VRAI MÉCANISME, expliqué étape par étape, chaque plan apportant une information neuve.",
  "• Plan 6 : LE « SAUF QUE » — le basculement. Juste avant ou juste après, une RELANCE en question très courte suivie de sa réponse sèche, phrases nominales assumées : « Et le trou au milieu du front ? Pas un œil. La cavité de la trompe. »",
  "• Plan 7 : LA CONSÉQUENCE CONCRÈTE — ce que ça change, ce qu'on voit, ce qu'on ressent.",
  "• Plan 8 : LA CHUTE — elle REFORMULE le sujet en UNE phrase qu'on a envie de répéter à quelqu'un. Jamais une morale, jamais « comme quoi… », jamais « le saviez-vous ? », jamais un résumé.",
  "CHUTES DE RÉFÉRENCE VALIDÉES (le niveau exact à atteindre) : « Il ne fait pas chaud parce qu'on est près du Soleil, mais parce qu'on est penché vers lui. » · « L'automne ne peint pas les arbres. Il enlève juste la couche de peinture verte. » · « Tes frissons sont le geste d'un animal que tu n'es plus depuis un million d'années. » · « Une nuit blanche, ce n'est pas de la fatigue. C'est un cerveau qu'on n'a pas rincé. » · « Le monstre le plus célèbre de la mythologie est une erreur de paléontologie commise 2 500 ans avant son invention. »",
...
/** Consignes d'écriture ajoutées au prompt v1 pour le style papier v2. */
export const V2_WRITING_BRIEF =
  "ACCROCHE — LE SPECTATEUR NE CONNAÎT RIEN (règle éliminatoire, elle prime sur tout le reste). Celui qui tombe sur la vidéo scrolle depuis vingt minutes, il n'a jamais entendu parler du sujet et il ne connaît pas la chaîne. INTERDIT : une accroche qui s'appuie sur un événement, un lieu, une œuvre ou des gens que le spectateur devrait déjà connaître — « Seize rugbymen ont survécu 72 jours dans les Andes », « Le talon d'Achille n'existe pas chez Homère » : si la première phrase suppose qu'il sache de quoi on parle, elle est ratée. À LA PLACE, la première phrase le met DANS la situation, avec une image qu'il se représente immédiatement, et elle contient déjà le danger, la perte ou l'anomalie. Même sujet, accroche qui marche : « Ton avion s'écrase en pleine montagne. Vous êtes vingt-neuf vivants, et dans dix jours la radio annoncera que les secours ont abandonné. » Les noms, les dates et le lieu viennent APRÈS, une fois qu'il est accroché. TEST À APPLIQUER AVANT DE RÉPONDRE : lis la première phrase à quelqu'un qui ne connaît pas le sujet — s'il doit demander « c'est quoi ? » ou « et alors ? », tu la réécris.\n" +
  "REGISTRE DU SCRIPT, décidé par le sujet AVANT d'écrire (tu le nommes en un mot au début de hookChoice) — le ton, la personne et le temps changent avec lui ; la structure, les règles d'accroche et le niveau de langue ne changent jamais :\n" +
  "• INTIME — le sujet parle du corps, de la tête, de la mémoire, des émotions, de la mort ou du temps du spectateur : « tu » du début à la fin, au présent ; le spectateur est le personnage ; au moins une phrase le renvoie à un moment précis de sa propre vie (« Souviens-toi d'un été quand tu avais huit ans », « la dernière fois que tu as cherché un mot ») ; la chute se retourne vers lui (« Tu ne peux pas ajouter des années à ta vie, mais tu peux rendre chaque année plus longue. »).\n" +
  "• VERTIGE — conscience, univers, infini, mort, temps : phrases un peu plus amples, « imagine », une seule idée par phrase, on laisse chaque idée s'installer ; on ne rassure pas et on ne conclut pas par une morale : la dernière phrase est le paradoxe lui-même (« Alors peut-être que tu es immortel. Mais tu seras le seul à le savoir. »).\n" +
  "• RÉCIT — un événement, des gens, un lieu : au passé (passé composé et imparfait), on suit une personne ou un groupe précis, dans l'ordre où ça s'est passé, avec ce qu'ils risquaient ; la chute dit ce qu'il en reste aujourd'hui.\n" +
  "• MÉCANISME — comment une chose de la nature ou du corps fonctionne : au présent, « il », « elle » ou « ton corps », étape par étape, chaque étape comparée à quelque chose de familier ; la chute est la conséquence qu'on n'attendait pas.\n" +
  "• MYTHE — une histoire que tout le monde connaît et sa vraie origine : on part de l'histoire telle que tout le monde la raconte, puis « sauf que » et les faits, au passé ; la chute remet l'histoire et la réalité face à face.\n" +
  "Un sujet peut mêler deux registres (INTIME + MÉCANISME pour « pourquoi tu bâilles ») : le premier donne la voix, le second la structure. Cette règle prime sur le style narratif. On ne mélange jamais passé et présent dans une même partie du récit.\n" +
  "TEST DE L'AMI (chaque phrase, avant de répondre) : tu lis la phrase à voix haute comme si tu la disais à un ami en face de toi. Si elle sonne comme un documentaire télé, un manuel ou une voix off de musée, tu la réécris. INTERDITS, et tous les tics du même genre : commencer une phrase par une date, une année ou une époque (« Cette nuit de 1986 », « En 1986 », « À cette époque », « Ce jour-là ») ; « il faut savoir que », « on estime que », « ce qu'on appelle », « en réalité », « notamment ». On ne date jamais un fait par une année en chiffres : si un repère dans le temps est utile, il se donne une fois, par une image que le spectateur vit (« il y a quarante ans, quand tes parents étaient petits »).\n" +
  "UNE SEULE IMAGE, TENUE DU DÉBUT À LA FIN (règle éliminatoire : c'est elle qui décide si on comprend ou pas). Tu choisis UNE façon de montrer la chose et tu n'en introduis JAMAIS une deuxième. Un script qui parle d'un frein, puis d'un moteur de fusée, puis d'un chef d'orchestre, puis de câbles blindés, puis d'un bolide oblige le spectateur à repartir de zéro cinq fois : il décroche même si chaque mot est simple.\n" +
  "INTERDIT : LES COMPARAISONS MÉCANIQUES, ET LES MOTS QUI VONT AVEC. Liste de mots bannis, quelle qu'en soit la tournure : frein, freiner, frein d'urgence, volant, au volant, moteur, bolide, accélérateur, câble, câblage, circuit, interrupteur, bouton, machine, ordinateur, disque dur, programme, blindage, chef d'orchestre, système de contrôle, centre de commande. Une tête ne contient ni frein ni volant : « la zone qui appuie sur le frein » ne veut rien dire et ne s'imagine pas. Si un de ces mots apparaît dans une phrase, la phrase est fausse — tu la réécris, tu ne cherches pas une autre comparaison.\n" +
  "À LA PLACE, TU DIS CE QUE LA CHOSE FAIT, QUAND, ET CE QUE ÇA DONNE POUR LE SPECTATEUR. Une zone du cerveau ne « freine » pas : elle envoie un signal, et ce signal arrive tôt ou tard. Donc on écrit le moment, pas la mécanique. Exemple complet, sur « ton cerveau n'est pas fini à vingt ans » : « Tu envoies le message à deux heures du matin. Le « c'était con » arrive le lendemain matin. Cette pensée-là vient d'une zone juste derrière ton front. Chez toi, elle met encore des années à finir de se brancher, alors elle arrive en retard. » — aucune comparaison, aucun mot technique, et pourtant on a compris. AVANT D'ÉCRIRE, tu cherches LE moment que le spectateur a vécu (le message à deux heures du matin, les jambes en plomb quand on te poursuit dans un rêve) ; tu le racontes comme une scène, l'explication vient se coller dessus, et la chute y revient.\n" +
  "ENSUITE LES MÊMES MOTS REVIENNENT : une chose = un nom, du début à la fin. Si tu l'as appelée « cette zone », c'est « cette zone » jusqu'au bout, jamais « le système de contrôle » ni « le blindage » trois phrases plus loin.\n" +
  "CHAQUE PHRASE REPREND UN MOT DE LA PRÉCÉDENTE. C'est ce qui donne l'impression de suivre une histoire et pas une liste. Une phrase qui ne partage aucun mot avec celle d'avant arrive de nulle part : réécris-la.\n" +
  "LE SUJET DE CHAQUE PHRASE EST UNE CHOSE QU'ON PEUT VOIR — toi, ton cerveau, cette zone, le fil, le produit chimique. Jamais une abstraction qui agit toute seule (« le cerveau humain se construit d'arrière en avant », « ce système de contrôle ») : personne ne peut se la représenter.\n" +
  "UN SEUL MOT TECHNIQUE PAR VIDÉO, ET MIEUX VAUT ZÉRO : si « la zone juste derrière ton front » suffit, tu n'as pas besoin de son nom savant.\n" +
...
export const V2_STORYBOARD_BRIEF =
  ...
  "\n" +
  "STYLE (identique sur tous les plans) : handmade layered paper cut-out diorama, 4 to 6 stacked planes of matte construction paper with torn deckled edges, visible paper grain, crisp cast shadows between the layers, one warm directional key light from the upper left. La scène remplit tout le carré jusqu'aux bords : jamais de cadre, de boîte, de bordure ni de contour déchiré autour de la scène. PALETTE fixée une fois pour toute la vidéo : deux tons de papier dominants + un crème + UN SEUL rouge vif ; tu l'écris dans le champ palette et tu la rappelles dans chaque imagePrompt. Les humains sont TOUJOURS des silhouettes de papier plates SANS VISAGE (no eyes, no mouth, no facial features) ; quand une personne apparaît, elle EST l'élément rouge du plan. Un seul élément rouge par plan, nommé dans chaque imagePrompt. Aucun texte, lettre, chiffre ou logo dans l'image.\n" +
  "\n" +
  "CE QU'ON VOIT = CE QU'ON ENTEND (règle n°1 du storyboard) : l'image de chaque plan montre LITTÉRALEMENT le sujet de la phrase — la chose ou la personne dont on parle, au moment où on en parle. « Le couvercle de béton s'est soulevé » → on voit le couvercle qui se soulève ; « l'air est entré dans la cuve » → la cuve ouverte et l'air qui s'y engouffre ; « des hélicoptères ont jeté du sable » → un hélicoptère qui lâche du sable. Une COMPARAISON n'est JAMAIS illustrée : « lourd comme trois cents éléphants » → on montre le couvercle, pas des éléphants ; « comme une cheminée géante » → le feu qui monte, pas une cheminée. TEST : quelqu'un qui regarde sans le son doit pouvoir dire de quoi parle la phrase. La métaphore n'est permise que pour une idée abstraite sans objet visible (le temps, la mémoire, une probabilité), et jamais sur deux plans de suite.\n" +
  "\n" +
  "UN SEUL SUJET PAR PLAN (règle éliminatoire) : DEUX objets en papier au maximum, énormes, au centre du carré. INTERDIT : les scènes de foule, les décors chargés, les plans où le spectateur doit chercher ce qu'il faut regarder — un avion PLUS un hélicoptère PLUS dix silhouettes dans la même image, c'est raté. Quand la phrase parle d'un groupe, on montre UNE silhouette rouge et au plus deux silhouettes sombres derrière, jamais dix. Le spectateur doit pouvoir dire en un tiers de seconde ce qu'il regarde et le relier à la phrase qu'il entend. ZONE UTILE : les applications recouvrent le bord droit et le bas de l'image avec leurs boutons, donc le sujet, l'action et tout ce qui compte tiennent dans les DEUX TIERS CENTRAUX du carré ; les bords ne portent que du décor.\n" +
  "\n" +
  "UN SEUL MOUVEMENT PAR PLAN, ET C'EST L'ACTION DE LA PHRASE : ce qui bouge, c'est ce que la phrase dit qu'il se passe — le couvercle se soulève, l'air s'engouffre, le feu prend, le sable tombe, les hommes pellettent. Rien ne bouge pour décorer : pas de couches qui claquent, pas de fouetté de caméra, pas d'objet qui traverse le cadre si la phrase ne le raconte pas. La caméra accompagne discrètement (léger push-in, pan qui suit l'objet). L'intensité suit le sens : révélation, chiffre, danger, choc → RAPIDE ; mécanisme, déplacement, transformation → MOYEN et continu ; conclusion, émotion → LENT et ample. Chaque videoPrompt commence par « FAST: », « STEADY: » ou « SLOW: », n'anime que ce qui est dans l'imagePrompt, en 8 secondes maximum, et ce mouvement doit être visible dès la première seconde.\n" +
  "\n" +
  "CONTINUITÉ (un film, pas un diaporama) : quand deux phrases qui se suivent se passent au même endroit, on reste dans le même décor avec les mêmes objets et on ne change que le cadrage ou l'échelle (le réacteur en plan large → son couvercle en gros plan → l'intérieur de la cuve). Un objet ou un personnage introduit dans un plan garde exactement la même forme et les mêmes couleurs dans tous les plans suivants — décris-le avec les mêmes mots. On ne change de décor que quand l'histoire change de lieu, et alors seulement on termine le videoPrompt par une SORTIE (le sujet quitte le cadre, une couche recouvre la scène) pour que la coupe fasse transition. Le spectateur doit sentir qu'il suit UNE histoire qui avance, jamais une suite d'images.\n" +
  "\n" +
  "REGISTRE : si le script parle au spectateur (« tu »), la silhouette rouge EST le spectateur et les décors sont les siens — une chambre, un lit, un miroir, un téléphone, une fenêtre, une rue la nuit — jamais un laboratoire ni une planche d'anatomie ; pour un récit, la silhouette rouge est la personne qu'on suit ; pour un mécanisme de la nature, l'objet lui-même tient le rôle principal.\n" +
  "\n" +
  "PLAN 1 = L'ACCROCHE VISIBLE : son image dit le sujet SANS LES MOTS — un spectateur qui coupe le son comprend en un tiers de seconde de quoi parle la vidéo. Un seul sujet, énorme, lisible sur un téléphone. Son videoPrompt contient L'ACTION LA PLUS FORTE DE TOUTE LA VIDÉO et commence à la première image : quelque chose tombe, s'ouvre, éclate ou surgit dans le cadre.\n" +
  "\n" +
  "VARIÉTÉ OBLIGATOIRE : deux plans qui se suivent ne partagent jamais le même cadrage ni la même échelle — la variété vient du cadrage, jamais d'un changement de décor ou d'objet que l'histoire ne demande pas. Fais tourner, dans l'ordre qui colle au texte : PAYSAGE LARGE avec horizon et silhouette minuscule · OBJET SEUL énorme sur fond de papier uni · TRÈS GROS PLAN sur un détail qui remplit le carré · VUE DU DESSUS à plat · SILHOUETTE À CONTRE-JOUR devant un fond lumineux · COUPE ou SCHÉMA en couches de papier · SCÈNE DE NUIT (fond sombre) alternée avec SCÈNE DE JOUR (fond clair). Jamais de noms propres d'œuvres, de marques ou de personnages protégés dans les prompts : décris ce qu'on voit.\n" +
  "\n" +
  'Réponds uniquement en JSON : {"palette":string,"shots":[{"index":number,"overlay":string,"imagePrompt":string,"videoPrompt":string}]} — exactement un objet par plan reçu, dans le même ordre, avec le même index.';
