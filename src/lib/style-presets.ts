...
export const V2_SHOT_BRIEF =
  "PLANS (papier v2) : UN PLAN PAR PHRASE de la narration — une phrase = un plan = une image. Chaque imagePrompt décrit UNE MÉTAPHORE VISUELLE de la phrase, jamais une personne en train de vivre l'action : une idée abstraite devient un objet en papier. Les personnages sont TOUJOURS des silhouettes de papier SANS VISAGE ; jamais de gros plan sur un visage.\n" +
  "\n" +
  "PLAN 1 = L'ACCROCHE. Sa narration suit EXACTEMENT les règles d'accroche (deux phrases courtes, 20 à 30 mots, phrase 1 retire la croyance, phrase 2 annonce la vraie piste) — la limite de mots des autres plans ne s'applique PAS ici ; si l'un des hooks de référence validés porte sur ce sujet, tu le reprends mot pour mot. Son image doit dire le sujet SANS LES MOTS : un spectateur qui coupe le son comprend en un tiers de seconde de quoi parle la vidéo (pour « pourquoi rêve-t-on » : une silhouette rouge endormie dont la tête s'ouvre comme un tiroir plein de photos). Un seul sujet, énorme, lisible sur un téléphone. Son videoPrompt contient L'ACTION LA PLUS FORTE DE TOUTE LA VIDÉO, qui commence à la première image : quelque chose tombe, s'ouvre, éclate ou surgit dans le cadre.\n" +
  "\n" +
  "PLANS 2 À 4 = LE RÉCIT COMMENCE (règle éliminatoire). Ce ne sont JAMAIS une liste de preuves ou de constats posés côte à côte. C'est le début d'une histoire qu'on raconte à un ami : le plan 2 pose le personnage ou le lieu et ce qu'il faisait, le plan 3 dit ce qui lui arrive, le plan 4 ce que ça change — chaque phrase découle de la précédente (« et donc », « alors », « jusqu'au jour où », « sauf que »), et quelqu'un qui n'entend que ces trois phrases à la suite doit pouvoir les raconter à son tour. Une seule preuve concrète suffit, intégrée au récit, jamais quatre à la suite.\n" +
  "\n" +
  "TEMPS DU RÉCIT, décidé par le sujet (cette règle prime sur le style narratif) : un fait passé — histoire, mythe, invention, découverte, catastrophe — se raconte AU PASSÉ (passé composé et imparfait : « Achille portait une armure », « il a saigné »), jamais au présent ; un mécanisme encore vrai aujourd'hui — corps, nature, physique, habitude — se raconte AU PRÉSENT (« ton cerveau se nettoie »). On ne mélange pas les deux dans une même partie du récit.\n" +
  "\n" +
  "À partir du plan 2, chaque narration fait UNE seule phrase complète de 8 à 13 mots, avec un sujet, un verbe et une idée qui tient debout seule — jamais une phrase coupée pour tenir le compte.\n" +
  "\n" +
  "TEST DE L'AMI (chaque phrase, avant de répondre) : tu lis la phrase à voix haute comme si tu la disais à un ami en face de toi. Si elle sonne comme un documentaire télé, un manuel ou une voix off de musée, tu la réécris. INTERDITS, et tous les tics du même genre : commencer une phrase par une date, une année ou une époque (« Cette nuit de 1986 », « En 1986 », « À cette époque », « Ce jour-là », « Quelques heures plus tard ») ; « il faut savoir que », « on estime que », « ce qu'on appelle », « en réalité », « notamment » ; toute phrase qui commence par un complément de temps ou de lieu au lieu d'un sujet qui agit. On ne date jamais un fait : si un repère dans le temps est vraiment utile, il se donne UNE fois, par une image que le spectateur vit (« il y a quarante ans, quand tes parents étaient petits »), jamais par un chiffre d'année. Exemple réel à ne plus produire : « Cette nuit de 1986, le réacteur a projeté son toit de mille tonnes en l'air. » → à écrire : « Le réacteur a soufflé son toit de mille tonnes comme un couvercle de casserole. » Chaque phrase commence par ce qui agit (le réacteur, la lave, les trois hommes, tu) et dit ce que ça fait.\n" +
  "\n" +
  "ON APPREND ET ON RESSENT (règle éliminatoire, les deux à la fois) : le spectateur doit repartir avec TROIS choses concrètes qu'il pourra répéter le soir même — un chiffre comparé à quelque chose de familier, un mécanisme expliqué, un fait qu'il ne connaissait pas — ET il doit avoir ressenti quelque chose. Pour ça, le récit suit toujours un ENJEU HUMAIN concret : quelqu'un risque, choisit, perd, se trompe ou est sauvé ; au moins un plan met le spectateur à sa place (« tu », « imagine que tu tiens la lampe »), et la chute touche autant qu'elle apprend. Un script qui n'apprend rien se scrolle ; un script qui ne fait rien ressentir s'oublie.\n" +
  "\n" +
  "MOUVEMENT DÉCIDÉ PAR LE SENS, plan par plan, dans videoPrompt : une phrase qui révèle, qui donne un chiffre, un danger ou un choc → mouvement RAPIDE (chute, éclatement, zoom brusque, fouetté de caméra, couches qui claquent) ; une phrase qui explique un mécanisme → mouvement MOYEN et continu (une chose se déplace, s'ouvre, se transforme sous nos yeux) ; une phrase de conclusion ou d'émotion → mouvement LENT et ample. Quand la phrase enchaîne directement sur la suivante, termine videoPrompt par une SORTIE (le sujet quitte le cadre, une couche recouvre la scène, la caméra dépasse le sujet) pour que la coupe fasse transition. Chaque videoPrompt nomme explicitement son intensité : « FAST: … », « STEADY: … » ou « SLOW: … » en premier mot. QUOTA : au moins CINQ plans « FAST: » par vidéo et jamais plus de TROIS « SLOW: » ; dans chaque videoPrompt, y compris les SLOW, un objet ou une couche entière de papier TRAVERSE au moins un tiers du carré (quelque chose tombe, glisse, s'ouvre, grandit, passe devant la caméra) et la caméra bouge aussi — une simple lumière qui change ou un parallaxe seul ne compte pas comme mouvement.\n" +
  "\n" +
  "VARIÉTÉ OBLIGATOIRE : deux plans qui se suivent ne partagent JAMAIS le même cadrage ni la même échelle. Fais tourner ces cadrages dans l'ordre qui colle au texte, sans jamais répéter le précédent : PAYSAGE LARGE avec horizon, la silhouette minuscule dedans · OBJET SEUL, énorme, isolé sur un fond de papier uni · TRÈS GROS PLAN sur un détail qui remplit tout le carré · VUE DU DESSUS à plat · SILHOUETTE À CONTRE-JOUR devant un fond lumineux · COUPE ou SCHÉMA en couches de papier · SCÈNE DE NUIT (fond sombre) alternée avec SCÈNE DE JOUR (fond clair). Le décor remplit tout le carré jusqu'aux bords : jamais de cadre, de boîte, de bordure ou de contour déchiré autour de la scène — le papier file hors des quatre bords. La palette (deux tons de papier + crème + un seul rouge) et la lumière restent les mêmes sur toute la vidéo : note-les dans le champ palette du script.";
